...
# --- Función de validación para IPv6 optimizada ---
...
# --- Archivo CSV de salida IPv6 ---
csv_out = open(f"resultados_{DESCRIPCION_PEER.lower().replace(' ', '_')}_ipv6.csv", "w", newline="")
...
# --- Contar rutas IPv6 entregadas por ese peer ---
with open("rib.txt") as f:
    total = sum(
        ...
        and ":" in l.strip().split("|")[5]  # solo prefijos IPv6
    )

# --- Procesar rutas IPv6 ---
with open("rib.txt") as f, tqdm(total=total, desc=f"Rutas IPv6 de {DESCRIPCION_PEER}") as pbar:
    for line in f:
...
print(f"\nResultados para rutas IPv6 recibidas desde {DESCRIPCION_PEER}:")
